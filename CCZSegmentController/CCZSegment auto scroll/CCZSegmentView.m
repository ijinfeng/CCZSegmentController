...
typedef void(^CCZIndexBlock)(NSUInteger ,UIButton *);
int const CCZSegmentHeight = 44;
@interface CCZSegmentView ()
...
@property (nonatomic, copy)   CCZIndexBlock indexBlock;
...
- (void)segmentBasicSetting {
    ...
    _segmentHeight = CCZSegmentHeight;
    _minItemSpace = 20.;
...
- (void)segmentPageSetting {
    ...
    _separateLine.backgroundColor = _segmentTintColor;
    ...
    for (int i = 0; i < _titles.count; i++) {
        ...
        if (i == 0) {
            ...
            // 添加指示杆
            _indicateView.frame = CGRectMake(_buttonSpace, _segmentHeight - _indicateHeight, titleSize.width, _indicateHeight);
            [_contentView addSubview:_indicateView];
        }
    }
    
    self.contentView.contentSize = CGSizeMake(item_x, _segmentHeight);
    _separateLine.frame = CGRectMake(0, _segmentHeight - 1, self.contentView.contentSize.width, 1);
}
...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:_duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (_style == CCZSegmentStyleDefault) {
...
- (void)adjustOffsetXToFixIndicatePosition:(CGFloat)offsetX {
    ...
    CGFloat originMovedX = _style == CCZSegmentStyleDefault? (CGRectGetMinX(_selectedButton.frame) + _buttonSpace) : CGRectGetMinX(_selectedButton.frame);
    CGFloat targetMovedWidth = [self widthAtIndex:currentIndex];//需要移动的距离
    
    CGFloat targetButtonWidth = _style == CCZSegmentStyleDefault? ([self widthAtIndex:buttonIndex] - 2 * _buttonSpace) : [self widthAtIndex:buttonIndex]; // 这个会影响width
    CGFloat originButtonWidth = _style == CCZSegmentStyleDefault? ([self widthAtIndex:[self index]] - 2 * _buttonSpace) : [self widthAtIndex:[self index]];
...
- (void)setSegmentTintColor:(UIColor *)segmentTintColor {
    ...
    _separateLine.backgroundColor = segmentTintColor;
    for (UIView *view in _contentView.subviews) {
...
- (void)setStyle:(CCZSegmentStyle)style {
    _style = style;
    
    if (CCZSegmentStyleFlush) {
