...
#define CCZBadgeTag 1104
#define CCZNumberLabelTag 1103
#define CCZNormalBadgeWidth 7
#define CCZNumberBadgeWidth 18 // 带数字的最小宽度

static const char *offsetSizeKey = "badgeSizeKey";
@implementation UIView (CCZBadge)
...
- (void)addNumberBadge:(NSInteger)number badgeOffsetSize:(CGSize)size color:(UIColor *)color borderColor:(UIColor *)bColor {
    ...
    numberLabel.tag = CCZNumberLabelTag;
...
- (void)numberChangeByAdd:(BOOL)add {
    UIView *badgeView = [self viewWithTag:CCZBadgeTag];
    if (!badgeView) {
        return;
    }
    
    UILabel *numberLabel = [badgeView viewWithTag:CCZNumberLabelTag];
    if (!numberLabel) {
        return;
    }
